...
const LOGO_SVG = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" width="100%" height="100%">
  ...
  <g>
    ...
    <rect x="146" y="248" width="220" height="118" rx="16" fill="url(#gold)" stroke="#124d36" stroke-width="6"/>
...
export default function DashboardHeader({ userEmail }: Props) {
  ...
  return (
    <div style={{
      ...
      padding: "10px 20px", background: "var(--panel)", borderBottom: "1px solid var(--line)",
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 10, cursor: "pointer" }}>
        ...
        <div>
          <div style={{ fontSize: 14, fontWeight: 800, letterSpacing: "-0.3px", lineHeight: 1.1 }}>Aye Aye Trader</div>
          <div style={{ fontSize: 10, color: "var(--mut)", letterSpacing: "0.5px" }}>Process over P&L</div>
        </div>
      </div>

      {/* Right side */}
      <div style={{ display: "flex", alignItems: "center", gap: 12, fontSize: 12, color: "var(--mut)" }}>
        <span>{userEmail}</span>
        <button onClick={signOut} style={{
          background: "transparent", border: "1px solid var(--line)", borderRadius: 6,
          color: "var(--mut)", padding: "4px 10px", cursor: "pointer", fontSize: 12, transition: ".12s",
        }}
          onMouseOver={(e) => { const b = e.currentTarget; b.style.borderColor = "var(--red)"; b.style.color = "var(--red)"; }}
          onMouseOut={(e) => { const b = e.currentTarget; b.style.borderColor = "var(--line)"; b.style.color = "var(--mut)"; }}
        >
          Sign out
        </button>
      </div>
